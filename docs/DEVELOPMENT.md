...
EF Core tooling is pinned in `.config/dotnet-tools.json`. After a model change, generate migrations with the Infrastructure project and API startup project, then run `dotnet ef migrations has-pending-model-changes` with the same project pair. Migration integration tests require Docker and apply the schema to an ephemeral PostgreSQL 17 container. `dotnet run --project src/ArrControl.Api -- database migrate` (or `dotnet ArrControl.Api.dll database migrate`) is the only application-owned migration command; it takes a PostgreSQL advisory lock, applies all pending migrations, verifies none remain, and returns nonzero on failure. Normal API startup never applies migrations automatically. Compose runs this command in the one-shot `migrate` service before `app`.

For a fresh local-identity database, apply migrations first and set both `Bootstrap__AdminEmail` and `Bootstrap__AdminPassword` for one startup. The password must be at least 12 characters and must not be the `.env.example` placeholder. Remove both settings after the bootstrap-complete log entry; retaining or restoring them cannot create another administrator because completion is persisted. Secure cookies are unconditional, so exercise browser authentication through HTTPS. The flow is `GET /api/v1/auth/csrf`, then send the returned token as `X-CSRF-Token` with its cookie on login, refresh, and logout; use the replacement token returned after login or refresh.
...
The Compose application port is HTTP-only and Secure `__Host-` cookies are intentionally unusable there. One local option is Caddy with its development CA. Keep Compose listening on port 8080, install Caddy, set `ARRCONTROL_PUBLIC_URL=https://arrcontrol.localhost` in `.env`, and use this `Caddyfile`:
